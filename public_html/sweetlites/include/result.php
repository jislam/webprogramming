    ...
    $sql = "SELECT prod_id, prod_name, prod_price, prod_thumbnail, prod_qty, c.cat_id
            ...
            WHERE pd.cat_id = c.cat_id AND pd.cat_id IN $children AND prod_name LIKE '%$terms%'
            ORDER BY prod_name";

    $stmt = $conn->prepare(getPagingQuery($sql, $productsPerPage));
    $stmt->execute();
    $result = $stmt->get_result();
    $pagingLink = getPagingLinkForSearch($sql, $productsPerPage, "c=$catId");
