...
function checkUser()
{
	...
	if (!isset($_SESSION['plaincart_admin_id'])) {
...
function doLogin()
{
	...
	if ($userName == '') {
		...
		/*$sql = "SELECT admin_id
		        FROM user
				WHERE admin_name = '$userName' AND admin_pass = PASSWORD('$password')";*/
				$sql = "SELECT admin_id
		        FROM admin 
				WHERE admin_name = '$userName' AND admin_pass = '$password'";
		...
		if ($result->num_rows == 1) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                $_SESSION['plaincart_admin_id'] = $admin_id;
...
function doLogout()
{
    ...
    if (isset($_SESSION['plaincart_admin_id'])) {
		unset($_SESSION['plaincart_admin_id']);
	}
...
{
	$sql = "SELECT cat_id, parent_id, cat_name FROM tbl_category where cat_publish='Y' ORDER BY cat_id";
